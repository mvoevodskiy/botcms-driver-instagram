...
class Instagram {
                ...
                for (let thread of threads) {
                    if (thread.last_activity_at > storedinbox.last) {
                        for (let msg of thread.items) {
                            if (!thread.is_group) {
                                this.users2directThreads[msg.user_id] = thread.thread_id;
                            }
    ...
    async defaultCallback (t, update) {
        // console.log(update);
        ...
        let SM = new SessionManager({IG: t});
        // console.log(SM);
    ...
    send (Parcel) {
        ...
        return (async () => {
            console.log(Parcel);
            let threadId = this.users2directThreads[Parcel.peerId] === undefined ? Parcel.peerId : this.users2directThreads[Parcel.peerId];
            const thread = this.Transport.entity.directThread(threadId);
    ...
    async launch(middleware, ...middlewares) {
        Bluebird.try(async () => {
            (async () => {
                ...
                fbnsClient.on('direct_v2_pending', data => this.handleDirect(this, data));
    ...
    async handleDirect (t, data) {
        console.info('IG HANDLE DIRECT. ', data);
    ...
    async handleDirectPending (t) {
        let timeout = Math.random() * 120000;
        setTimeout(() => {
            console.log('NEW ROUND');
            const inboxFeed = t.Transport.feed.directPending();
            // const threads = await inboxFeed.items();
            inboxFeed.items$.subscribe(
                threads => {
                    console.log(threads);
                    for (let thread of threads) {
                        t.handleDirect(t, {
                            ...
                            message: thread.last_permanent_item.text,
                        })
                    }
                }
            );
            t.handleDirectPending(t);
        }, timeout);
    }
    ...
    storeGet (key) {
        // console.log('BOTCMS INSTAGRAM DRIVER STORE GET. KEY ' + key);
    ...
    storeSet (key, value) {
        let primitive = {};
        // console.log('BOTCMS INSTAGRAM DRIVER STORE SET ' + key + ', VALUE ', value);
        for (let k in value) {
